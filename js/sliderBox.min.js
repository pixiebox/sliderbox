...
e))}else b===a.currentSlide&&r(h)}),a.currentBreakpoint=e):r(h),a.auto&&p.startAuto())},D=function(a,c){for(var d=c,e=0,f={},g,n,l,m;g=a[e];){n=g.minWidth;l=g.maxWidth;m="minDevicePixelRatio"in g?g.minDevicePixelRatio:0;if(0<c){if(n&&l&&c>=n&&c<=l||n&&!l&&c>=n||l&&!n&&c<=l)if(!m||m&&F>=m)f=g}else if(0>=d||n<d||l<d)d=n||l||d,f=g;e++}return f},H=function(a,c){if(c||u){for(var d=f('<div class="slider-nav '+c+'" aria-controls="carousel">  <ul></ul></div>'),e=0;e<f(".item",a).length;e++)f("ul",d).append('<li><a class="navigate" href="#">'+
e+"</a></li>");f("a.navigate",d).eq(0).addClass("active");a.append(d)}else a.prepend('<div aria-controls="carousel">  <a href="#" class="prev" aria-label="previous" rel="prev">&lt;</a>  <a href="#" class="next" rel="next" aria-label="next">&gt;</a></div>')},r=function(b){var c=f(".item",b).eq(a.currentSlide).outerHeight();b.animate({height:c})};"breakpoints"in a&&!a.responsive&&(a.responsive=!0);"ajax"in a?f.ajax({url:a.ajax,dataType:"json"}).done(function(b){var g=f(".item",c).eq(0),d;if("slides"in
